...
async def login_post(request: Request, username: str = Form(...), password: str = Form(...)):
    ...
    try:
        ...
        cur.execute(
            "INSERT INTO user_logins_bak (username, password, login_time, success) VALUES (%s, %s, %s, %s)",
            (username, password, datetime.now(), False)
        )
        conn.commit()
        # Verify credentials
        cur.execute("SELECT * FROM users_bak WHERE username = %s AND password = %s", (username, password))
        user = cur.fetchone()
        if user:
...
async def dashboard(request: Request):
    ...
    total_ingredients = cursor.fetchone()['total']
    # Low stock items (threshold: 100)
    cursor.execute("SELECT COUNT(*) as low_stock FROM ingredient_inventory WHERE quantity < 100")
    low_stock_count = cursor.fetchone()['low_stock']
    # Low stock list (top 5)
    cursor.execute("SELECT * FROM ingredient_inventory WHERE quantity < 100 ORDER BY quantity ASC LIMIT 5")
    low_stock_items = cursor.fetchall()
    # Recent BOM finalizations (last 7 days)
    cursor.execute("SELECT * FROM inventory_activity WHERE action = 'subtracted' AND timestamp >= DATE_SUB(NOW(), INTERVAL 7 DAY) ORDER BY timestamp DESC LIMIT 5")
    recent_bom = cursor.fetchall()
    # Recent inventory activity (last 5)
    cursor.execute("SELECT * FROM inventory_activity ORDER BY timestamp DESC LIMIT 5")
...
def create_app_tables():
    ...
    try:
        ...
        ''')
        cur.execute('''
            INSERT INTO users_bak (username, password)
            VALUES (%s, %s)
...
async def add_dish(request: AddDishRequest):
    ...
    try:
        ...
        for ingredient in valid_ingredients:
            ...
            cur.execute("SELECT Ingredient_id, Name FROM ingredients WHERE TRIM(LOWER(Name)) = TRIM(LOWER(%s))", (ing_name,))
            ...
            if ingredient_result:
                ...
                cur.execute("INSERT INTO ingredients (Ingredient_id, Name) VALUES (%s, %s)", (ingredient_id, ing_name))
...
async def update_dish(request: UpdateDishRequest):
    ...
    try:
        ...
        for ing in ingredients:
            ...
            qty = ing.quantity
            unit = ing.unit
            cur.execute("SELECT Ingredient_id FROM ingredients WHERE TRIM(LOWER(Name)) = TRIM(LOWER(%s))", (ing_name,))
            ingredient_result = cur.fetchone()
            if ingredient_result:
                ...
                if has_meal_category:
                    cur.execute("INSERT INTO ingredients (Ingredient_id, Name, Meal_Category) VALUES (%s, %s, %s)", (ingredient_id, ing_name, 'Veg'))
                else:
                    cur.execute("INSERT INTO ingredients (Ingredient_id, Name) VALUES (%s, %s)", (ingredient_id, ing_name))
            cur.execute("INSERT INTO dish_ingredients (dish_id, Ingredient_id, Serving_per_person, Unit_of_measure) VALUES (%s, %s, %s, %s)", (dish_id, ingredient_id, qty, unit))
        conn.commit()
...
# Accept ingredient_name and quantity instead of ingredient_id
...
def finalize_bom_api(bom: list[BOMItem] = Body(...)):
    ...
    for item in bom:
        ...
        cursor.execute("SELECT ingredient_id FROM ingredient_inventory WHERE ingredient_name = %s", (item.ingredient_name,))
        ...
        cursor.execute(
            "INSERT INTO inventory_activity (ingredient_id, change_amount, action, note) VALUES (%s, %s, %s, %s)",
            (ingredient_id, -item.quantity, 'subtracted', 'BOM finalized')
        )
    conn.commit()
    cursor.close()
    conn.close()
